...
use App\Http\RepositoryProtocol\RankingGame;

class RankingGameRepository extends InitRepository implements RepositoryInterface
{
    use TryCatchTrait;

    public function __construct()
    {
        parent::__construct(new RankingGame());
    }
    ...
    public function index($int_type, $str_pCode)
    {
        return $this->selectTryCatch(function () use ($int_type, $str_pCode) {
            return $this->model->select('game_promotion_id', 'data')
                ->join('game_promotion', 'ranking_game.game_promotion_id', '=', 'game_promotion.id')
    ...
    public function delete($int_typeID, $str_promoCode)
    {
        ...
        return $this->queryTryCatch(function () use ($int_typeID, $str_promoCode) {
            return $this->model->join('game_promotion', 'ranking_game.game_promotion_id', '=', 'game_promotion.id')
